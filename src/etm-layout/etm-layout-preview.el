  ...
  (if (null windows)
    ...
    (let ((max-x 0)
      ...
      (let ((rows 1)
        ...
        (let
	    ((x-positions
	      (seq-uniq (mapcar (lambda (w) (nth 2 w)) windows))))
          (setq cols (length x-positions)))
        ;; Count rows by Y positions
        (let
	    ((y-positions
	      (seq-uniq (mapcar (lambda (w) (nth 3 w)) windows))))
  ...
  (condition-case err
      (with-temp-buffer
        ...
        (let ((name nil)
          ...
          (when
	      (re-search-forward
	       "defun etm-open-\\([^[:space:]()]+\\)" nil t)
            (setq name (match-string 1)))
          ;; Find the windows list
          (goto-char (point-min))
          (when
	      (re-search-forward "--etm-layout-create-from-positions"
				 nil t)
  ...
  (condition-case err
      (with-temp-buffer
        ...
        (let ((info (etm-layout-extract-info (buffer-string))))
          ...
          (plist-put info :timestamp
		     (nth 5 (file-attributes file-path)))
          info))
    (error
     (signal 'file-error
	     (list "Cannot parse layout file" file-path err)))))
...
(defun etm-layout-render-window-grid (windows dimensions)
  ...
  (let* ((rows (car dimensions))
    ...
    (push (concat "┌" (make-string col-width ?─)
                  (mapconcat
		   (lambda (_) (concat "┬" (make-string col-width ?─)))
    ...
    (dotimes (row rows)
      ...
      (push (concat "│" (make-string (1+ col-width) ? )
                    (mapconcat
		     (lambda (_)
		       (concat "│" (make-string (1+ col-width) ? )))
                     (number-sequence 2 cols) "")
                    "│")
	    grid-lines)
...
(defun etm-layout-render-preview (name windows host)
  ...
  (if (null windows)
    ...
    (let* ((dimensions (etm-layout-calculate-dimensions windows))
           (parsed-windows
	    (mapcar #'etm-layout-parse-window-spec windows))
           (grid
	    (etm-layout-render-window-grid parsed-windows dimensions)))
      (concat
       (format "Layout: %s%s\n"
               name
...
(define-derived-mode etm-layout-list-mode tabulated-list-mode
  "ETM-Layouts"
  "Major mode for ETM layout list.
Supports dired-like marking for batch operations."
